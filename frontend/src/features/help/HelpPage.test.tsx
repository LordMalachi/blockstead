...
test("searches the central task guides and links into the active server", () => {
  ...
  expect(screen.getByRole("heading", { name: "Protect and restore a world" })).toBeVisible();
...
test("asks the owner to choose a server when none is active", () => {
  ...
  const backupGuide = screen.getByRole("heading", { name: "Protect and restore a world" }).closest("article");
