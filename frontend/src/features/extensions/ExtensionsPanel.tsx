...
export function ExtensionsPanel({ profileId, stopped }: { profileId: string; stopped: boolean }) {
  ...
  return <section className="card" id="extensions"><div className="section-heading"><div><p className="eyebrow">Mods and plugins</p><h2>Extensions</h2></div><span>{view ? `${view.entries.length} active · ${view.disabled_entries.length} disabled` : "Loading"}</span></div>{unsupported ? <p className="empty-note">This server type does not load mods or plugins. Create a Fabric server for mods or a Paper server for plugins.</p> : <>{view?.warnings.map(warning => <div className="warning" key={`${warning.code}-${warning.files.join()}`}><strong>{warning.code.replace("-", " ")}</strong><span>{warning.message}</span><small>{warning.files.join(", ")}</small></div>)}<div className="extension-columns"><div><h3>Installed</h3><ul className="extension-list">{view?.entries.map(entry => <ExtensionRow key={entry.file_name} entry={entry} disabled={false} locked={!stopped || action.isPending} act={manage} />)}{view && !view.entries.length && <li className="empty-note">No active files in {view.directory}/.</li>}</ul></div><div><h3>Disabled</h3><ul className="extension-list">{view?.disabled_entries.map(entry => <ExtensionRow key={entry.file_name} entry={entry} disabled locked={!stopped || action.isPending} act={manage} />)}{view && !view.disabled_entries.length && <li className="empty-note">Nothing is disabled.</li>}</ul></div></div><div className="catalog-tools"><div><h3>Find on Modrinth</h3><form className="inline-form" onSubmit={search}><label>Search compatible projects<input value={query} onChange={event => setQuery(event.target.value)} placeholder="Lithium, LuckPerms…" /></label><Button disabled={!stopped || !query.trim()}>Search</Button></form>{results.isFetching && <p className="empty-note">Searching Modrinth…</p>}<div className="catalog-results">{results.data?.projects.map(project => <article key={project.project_id}><div><strong>{project.title ?? project.slug ?? project.project_id}</strong><p>{project.description}</p><small>{project.downloads?.toLocaleString() ?? "—"} downloads</small></div><Button className="button--secondary button--small" disabled={!stopped || action.isPending} onClick={() => action.mutate({ endpoint: `/profiles/${profileId}/extensions/install`, init: { method: "POST", body: JSON.stringify({ project_id: project.project_id }) } })}>Install</Button></article>)}</div></div><div><h3>Upload a jar</h3><form className="upload-form" onSubmit={upload}><label>Local .jar file<input name="file" type="file" accept=".jar,application/java-archive" required /></label><Button disabled={!stopped || action.isPending}>Upload</Button></form></div></div></>}{!stopped && !unsupported && <p className="muted-note">Stop the server before changing extension files.</p>}{notice && <p className={notice.startsWith("Change saved") ? "success" : "error"} role="status">{notice}</p>}</section>;
}
