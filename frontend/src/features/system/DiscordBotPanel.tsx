...
export function DiscordBotPanel() {
  ...
  return <section className="card" aria-labelledby="discord-bot-heading">
    ...
    {data && <>
      ...
      {data.relay_error && <p className="error" role="alert">{data.relay_error}</p>}
      {!data.relay_configured && !data.relay_error && <p className="warning" role="status">Set <code>BLOCKSTEAD_DISCORD_RELAY_URL</code> for the central relay and restart Blockstead. The Discord bot token belongs only on the relay.</p>}
