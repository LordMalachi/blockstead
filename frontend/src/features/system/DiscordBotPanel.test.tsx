...
test("shows pairing controls when the status API reports a configured relay", async () => {
  ...
  expect(await screen.findByRole("heading", { name: "Pair a server profile" })).toBeVisible();
  expect(screen.getByRole("button", { name: "Create pairing code" })).toBeVisible();
  expect(screen.queryByText(/Set BLOCKSTEAD_DISCORD_RELAY_URL/)).toBeNull();
  client.clear();
  vi.unstubAllGlobals();
});
...
test("shows an insecure relay URL as a configuration error", async () => {
  const invalid = {
    ...status,
    relay_configured: false,
    relay_url: null,
    relay_error: "BLOCKSTEAD_DISCORD_RELAY_URL must be an HTTPS relay origin",
    connections: [],
  };
  const fetchMock = vi.fn((url: string) => Promise.resolve(new Response(
    JSON.stringify(url.endsWith("/profiles") ? [] : invalid), { status: 200 },
  )));
  const client = renderPanel(fetchMock);

  expect(await screen.findByRole("alert")).toHaveTextContent("HTTPS relay origin");
