...
test("normalizes an uppercase custom server folder before creating the copy", async () => {
  const fetch = vi.fn().mockImplementation((url: string) => {
    const body = url.endsWith("/profiles")
      ? [{ id: "profile-1", name: "Family", server_directory: "/servers/family", distribution: "fabric", minecraft_version: "1.21.1", loader_version: "1", is_fixture: false }]
      : url.includes("/loader-migration/review")
        ? review
        : { id: "profile-2", name: "Family · Paper", distribution: "paper", minecraft_version: "1.21.1", loader_version: null, worlds_copied: review.worlds, source_directory: "/servers/family", destination_directory: "/servers/boney420-paper", world_copy_operations: review.world_copy_operations, source_profile_id: "profile-1", source_unchanged: true, extensions: review.extensions, next_route: "/servers/profile-2/mods?migration=1", eula_accepted: false };
    return Promise.resolve(new Response(JSON.stringify(body), { status: url.includes("/apply") ? 201 : 200, headers: { "Content-Type": "application/json" } }));
  });
  vi.stubGlobal("fetch", fetch);
  const client = new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } });
  ...
  fireEvent.change(screen.getByLabelText("New server folder"), { target: { value: "Boney420-Paper" } });
  expect(screen.getAllByText("/servers/boney420-paper").length).toBeGreaterThan(0);
  expect(screen.getByText(/saved as/)).toBeVisible();
  fireEvent.click(screen.getByRole("checkbox"));
  fireEvent.click(screen.getByRole("button", { name: "Create Paper copy" }));
