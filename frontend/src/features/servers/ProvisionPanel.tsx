...
export function ProvisionPanel({ stopped, onCreated }: { stopped: boolean; onCreated: (profileId: string) => void }) {
  ...
  return <section className="card onboarding-card" id="create-server">
    ...
    <form className="provision-form" onSubmit={submit}>
      ...
      <label>Profile name<input value={name} onChange={event => { setName(event.target.value); if (!directory || directory === normalizeServerDirectoryName(name)) setDirectory(normalizeServerDirectoryName(event.target.value)); }} required maxLength={80} /></label>
      <label>Server folder<input value={directory} onChange={event => setDirectory(event.target.value)} onBlur={() => setDirectory(current => normalizeServerDirectoryName(current))} pattern="[a-z0-9][a-z0-9_-]*" required maxLength={64} /></label>
