# Blockstead product specification

This is the complete product and engineering specification for Blockstead. It
defines the product vision, architecture, scope, security requirements, and
quality gates that govern every change.

If you want to install or run Blockstead, read the [README](../README.md)
instead — it covers setup, everyday use, updates, and removal on Linux Mint.
The current milestone roadmap lives in [update.md](../update.md).
...
The quick-start in the [README](../README.md) is the supported installation
path for Linux Mint 22.x and compatible Ubuntu-based systems. The installer
needs Python 3.12.x, Node.js with npm, `curl`, and `systemd`; Java 21 is
needed to run a modern vanilla Minecraft server. The installer checks for
these up front and offers to install any missing pieces through `apt` before
