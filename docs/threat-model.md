...
| Accidental data-folder deletion | data and servers are real host folders, not Docker-managed volumes, so no Docker or Compose command (including `down -v`) can remove them; the dashboard displays each file's exact on-disk folder next to it | A person with ordinary file-system access to this computer can still delete or move those folders directly, the same as any other files; external backups of both folders are required for disaster recovery |
...
| Leaked Discord webhook | webhook values are stored as application secrets, masked in the UI, excluded from audits and diagnostics, and never included in delivery history; owners can disable or remove the integration | Anyone who obtains the webhook can post to that Discord channel until it is rotated in Discord; Blockstead does not claim to revoke the provider-side credential |
| SSRF through Discord delivery | only HTTPS webhook URLs are accepted; DNS answers are checked for public addresses at configuration and delivery time; redirects are disabled; requests have bounded timeouts, durable retry limits, and redacted payloads | DNS can change after validation on a hostile network; use a trusted Discord webhook and review the delivery history when a provider changes its endpoint |
| Trusted helper overreach | viewer accounts are separate, centrally authorized, session-revocable, disabled-aware, and limited to operations summaries, roster, activity stories, backup metadata, alerts, performance, and World Care evidence; backend checks remain authoritative | A viewer can still see the operational facts intentionally granted and can use any permitted read-only evidence; owners should disable helpers when household access changes |
| Saved setup divergence or unsafe switch | variants use isolated profile folders and copied worlds, require a fresh verified source backup, preserve the source, delete incomplete targets, and re-review stop/start readiness before activation | A copied world can diverge immediately after creation; activation downtime and a failed target start leave both profiles stopped and require the owner to follow the recorded recovery guidance |
