...
def test_relay_url_requires_a_tls_origin(value: str) -> None:
    with pytest.raises(RelayError, match="HTTPS relay origin"):
        relay_url(Settings(_env_file=None, discord_relay_url=value))


def test_invalid_relay_url_disables_discord_without_exposing_a_credential() -> None:
    configuration = relay_configuration(
        Settings(_env_file=None, discord_relay_url="http://relay.example.test")
    )
    assert configuration["configured"] is False
    assert configuration["relay_url"] is None
    assert configuration["relay_error"] == (
        "BLOCKSTEAD_DISCORD_RELAY_URL must be an HTTPS relay origin"
    )
...
def test_relay_client_exposes_rotation_control_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = Settings(_env_file=None, discord_relay_url="https://relay.test")
