...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def backup_payload(record: BackupRecord, *, viewer: bool = False) -> dict[str, object]:
        ...
        if viewer:
            # A viewer can compare protection state and timing, but must not be
            # given an archive handle, manifest evidence, or an owner mutation
            # result. Keep the outcome fixed-map based so a malformed or
            # private helper error cannot carry a path/archive name over the
            # read-only boundary.
            viewer_result = {
            ...
            }.get(record.status, "Backup status recorded.")
            return {
                "id": record.id,
                "profile_id": record.profile_id,
                "status": record.status,
                "method": "world_archive",
                "trigger": "schedule" if record.trigger == "schedule" else "manual",
    ...
    def viewer_world_care_backup_payload(record: BackupRecord) -> dict[str, object]:
        """Keep World Care's backup object shape with all archive evidence blank."""

        return {
            **backup_payload(record, viewer=True),
            "file_name": None,
    ...
    def viewer_disk_payload(value: Mapping[str, object]) -> dict[str, object]:
        """Keep World Care's disk shape while blanking its filesystem path."""

        state = value.get("state")
        safe_state: str = (
            state
            if isinstance(state, str) and state in {"available", "missing", "unavailable"}
            else "unavailable"
        )
    ...
    def viewer_destination_check_payload(value: Mapping[str, object]) -> dict[str, object]:
        """Expose only fixed, non-location-specific destination evidence."""
        ...
        detail = {
            "available": "A private destination check completed.",
            "missing": "The destination is not currently available.",
            "unavailable": "The destination check was unavailable.",
        }[safe_state]
        checked_at = value.get("checked_at")
        safe_checked_at: str | None = None
        if isinstance(checked_at, str):
            try:
                ...
                safe_checked_at = checked_at
        return {
            "state": safe_state,
            "write_verified": (
                bool(value.get("write_verified")) if safe_state == "available" else False
            ),
            "read_verified": (
                bool(value.get("read_verified")) if safe_state == "available" else False
            ),
            "detail": detail,
            "checked_at": safe_checked_at,
        }

    def viewer_recovery_entries(entries: list[dict[str, object]]) -> list[dict[str, object]]:
        """Allow only the non-path recovery facts needed by the viewer."""
        ...
        for entry in entries:
            ...
            safe_label = (
                label
                if isinstance(label, str) and "/" not in label and "\\" not in label
                else "Recovery evidence"
            )
            state = entry.get("state")
            safe_state = state if state in {"available", "unknown"} else "unknown"
    ...
    def profile_world_care(profile_id: str, request: Request, db: Db) -> dict[str, object]:
        ...
        if admin.role == "viewer":
            server_disk = viewer_disk_payload(server_disk)
            for destination_payload in destinations:
                destination_payload["configured_path"] = ""
                destination_payload["disk"] = viewer_disk_payload(
                    cast(Mapping[str, object], destination_payload["disk"])
                )
                last_check = destination_payload.get("last_check")
                if isinstance(last_check, Mapping):
                    destination_payload["last_check"] = viewer_destination_check_payload(
                        last_check
                    )
            recovery = viewer_recovery_entries(recovery)
        return {
            ...
            "last_verified_backup": (
                (
                    viewer_world_care_backup_payload(latest)
                    if admin.role == "viewer"
                    else backup_payload(latest)
                )
                if latest
                else None
            ),
    ...
    def discord_bot_status(request: Request, db: Db) -> dict[str, object]:
        ...
        relay_configured = bool(relay_configuration(config).get("configured"))
        configuration = {
            **discord_config,
